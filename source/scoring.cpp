...
brown_basketball::scoring::score_league(
    const std::array<brown_basketball::common::team,
...
brown_basketball::scoring::impl::score_league(
  ...
  std::array<team_stats, brown_basketball::common::g_k_league_size>
      step1_team_stats{calc_team_stats(league)};
  if (ret_val == return_value_type::TEAM_STATS) {
    return step1_team_stats;
  }

  std::array<team_ranks, brown_basketball::common::g_k_league_size>
      step2_team_ranks{calc_team_ranks(step1_team_stats)};
  if (ret_val == return_value_type::TEAM_RANKS) {
    return step2_team_ranks;
  }

  std::array<team_scores, brown_basketball::common::g_k_league_size>
      step3_team_scores{calc_team_scores(step2_team_ranks)};
  if (ret_val == return_value_type::TEAM_SCORES) {
    return step3_team_scores;
  }

  std::array<team_total_score, brown_basketball::common::g_k_league_size>
      step4_team_total_score{calc_team_total_score(step3_team_scores)};
  if (ret_val == return_value_type::TEAM_TOTAL_SCORE) {
    return step4_team_total_score;
  }

  std::array<team_total_rank, brown_basketball::common::g_k_league_size>
      step5_team_total_rank{calc_team_total_rank(step4_team_total_score)};
...
brown_basketball::scoring::impl::calc_team_stats(
    const std::array<brown_basketball::common::team,
                     brown_basketball::common::g_k_league_size> &league) {
  std::array<team_stats, brown_basketball::common::g_k_league_size> ret_val{};
...
brown_basketball::scoring::impl::calc_team_ranks(
    const std::array<team_stats, brown_basketball::common::g_k_league_size>
        &league) {
  std::array<team_ranks, brown_basketball::common::g_k_league_size> ret_val{};
  for (std::size_t i_team{0}; i_team != league.size(); ++i_team) {
    ret_val[i_team].m_team = league[i_team].m_team;
  }

  std::array<std::size_t, brown_basketball::common::g_k_league_size> indices{};
...
brown_basketball::scoring::impl::calc_team_scores(
    const std::array<team_ranks, brown_basketball::common::g_k_league_size>
        &league) {
  std::array<team_scores, brown_basketball::common::g_k_league_size> ret_val{};

  constexpr auto get_points_for_rank{[](const int rank) -> int {
    return (brown_basketball::common::g_k_league_size - (rank - 1));
  }};
...
brown_basketball::scoring::impl::calc_team_total_score(
    const std::array<team_scores, brown_basketball::common::g_k_league_size>
        &league) {
  std::array<team_total_score, brown_basketball::common::g_k_league_size>
      ret_val{};
...
brown_basketball::scoring::impl::calc_team_total_rank(
    const std::array<team_total_score,
                     brown_basketball::common::g_k_league_size> &league) {
  std::array<team_total_rank, brown_basketball::common::g_k_league_size>
      ret_val{};
  for (std::size_t i_team{0}; i_team < league.size(); ++i_team) {
