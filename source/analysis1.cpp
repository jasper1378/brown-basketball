...
brown_basketball::analysis1::impl::accum_state_init_accum_state(
  ...
  assert(common::g_k_pool_size == database.size());
  ...
  for (std::size_t i_team{0}; i_team < common::g_k_league_size; ++i_team) {
    for (auto p_player{league[i_team].m_team->m_players.begin()};
      ...
      for (std::size_t i_cat{0};
        ...
        if (league[i_team].m_ranks[i_cat] == common::g_k_rank_top) {
          ++(accum_state[(*p_player)->m_idx].second.m_rank_counts[i_cat].m_top);
        }
        if (league[i_team].m_ranks[i_cat] <= common::g_k_rank_cutoff) {
...
brown_basketball::analysis1::impl::accum_state_read(
  ...
  assert(common::g_k_pool_size == accum_state.size());
  for (std::size_t i_player{0}; i_player < common::g_k_pool_size; ++i_player) {
